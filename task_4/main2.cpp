...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < N; i++) {
    const char* univ_from_class = moscow_univ[i].get_name_univ();
    if (strcmp(name_univ, univ_from_class) == 0) {
      const char* fac_from_class = moscow_univ[i].get_name_fac();
      size_t len = strlen(fac_from_class);
      size_t num = strspn(all_faculties_begin, fac_from_class);
      if (num != len) {
